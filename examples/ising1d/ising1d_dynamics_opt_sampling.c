...
int main()
{
	...
	// number of samples and random number generator
	const long nsamples = 16;
	...
	optimize_brickwall_circuit_hmat_sampling(ufunc, expiH, vlist_start, nlayers, nqubits, pperms, nsamples, &rng, &params, niter, f_iter, vlist_opt);
	...
	// exact target function value at optimized gates
	numeric f_ref;
	brickwall_unitary_target(ufunc, expiH, vlist_opt, nlayers, nqubits, pperms, &f_ref);
	printf("f_ref = %.12f\n", creal(f_ref));

	// save results to disk
	sprintf(filename, "../examples/ising1d/ising1d_dynamics_opt_sampling_n%i.hdf5", nlayers);
	...
	hsize_t frefdims[1] = { 1 };
	double f_ref_real = creal(f_ref);
	if (write_hdf5_dataset(file, "f_ref", 1, frefdims, H5T_IEEE_F64LE, H5T_NATIVE_DOUBLE, &f_ref_real) < 0) {
		fprintf(stderr, "writing 'f_ref' to disk failed\n");
		return -1;
	}
	// store parameters
