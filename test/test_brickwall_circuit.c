...
char* test_brickwall_unitary_grad_matfree()
{
	...
	{
		char filename[1024];
		sprintf_s(filename, 1024, "../../../test/data/test_brickwall_unitary_grad_matfree_V%i.dat", i);
		if (read_data(filename, Vlist[i].data, sizeof(numeric), 16) < 0) {
			return "reading two-qubit quantum gate entries from disk failed";
		}
	}
	...
	{
		char filename[1024];
		sprintf_s(filename, 1024, "../../../test/data/test_brickwall_unitary_grad_matfree_perm%i.dat", i);
		if (read_data(filename, perms[i], sizeof(int), L) < 0) {
			return "reading permutation data from disk failed";
		}
	}
	...
	for (int i = 0; i < 3; i++)
	{
		...
		char filename[1024];
		sprintf_s(filename, 1024, "../../../test/data/test_brickwall_unitary_grad_matfree_dVlist%i.dat", i);
